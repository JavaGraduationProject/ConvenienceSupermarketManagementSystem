...
<form name="form" action="<%=request.getContextPath()%>/dingdanServlet?method=list" method="post" >
        <div class="layui-row">
...
<td ><a href="<%=path%>/dingdanServlet?method=delete&keyid=<%=rs.getString("ddid")%>"  onClick="javascript:if(confirm('是否删除')){return true;} else{return false;}">删除</a>|<a href="<%=path%>/dingdanServlet?method=modifyindex&keyid=<%=rs.getString("ddid")%>">修改</a>|<a href="<%=path%>/dingdanServlet?method=detail&keyid=<%=rs.getString("ddid")%>">详细</a></td>
