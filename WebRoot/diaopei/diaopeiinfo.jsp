...
<td ><a href="<%=path%>/diaopeiServlet?method=delete&keyid=<%=rs.getString("dpid")%>"  onClick="javascript:if(confirm('是否删除')){return true;} else{return false;}">删除</a>|<a href="<%=path%>/diaopeiServlet?method=modifyindex&keyid=<%=rs.getString("dpid")%>">修改</a>|<a href="<%=path%>/diaopeiServlet?method=detail&keyid=<%=rs.getString("dpid")%>">详细</a></td>
