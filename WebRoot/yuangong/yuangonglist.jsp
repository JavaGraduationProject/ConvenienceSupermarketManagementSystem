...
<form name="form" action="<%=request.getContextPath()%>/yuangongServlet?method=list" method="post" >
        <div class="layui-row">
...
<td ><a href="<%=path%>/yuangongServlet?method=delete&keyid=<%=rs.getString("ygid")%>"  onClick="javascript:if(confirm('是否删除')){return true;} else{return false;}">删除</a>|<a href="<%=path%>/yuangongServlet?method=modifyindex&keyid=<%=rs.getString("ygid")%>">修改</a>|<a href="<%=path%>/yuangongServlet?method=detail&keyid=<%=rs.getString("ygid")%>">详细</a></td>
