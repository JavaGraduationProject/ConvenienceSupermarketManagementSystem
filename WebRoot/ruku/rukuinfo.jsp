...
<td ><a href="<%=path%>/rukuServlet?method=delete&keyid=<%=rs.getString("rkid")%>"  onClick="javascript:if(confirm('是否删除')){return true;} else{return false;}">删除</a>|<a href="<%=path%>/rukuServlet?method=modifyindex&keyid=<%=rs.getString("rkid")%>">修改</a>|<a href="<%=path%>/rukuServlet?method=detail&keyid=<%=rs.getString("rkid")%>">详细</a></td>
