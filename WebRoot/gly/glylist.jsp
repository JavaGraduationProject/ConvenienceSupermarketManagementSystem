...
<form name="form" action="<%=request.getContextPath()%>/glyServlet?method=list" method="post" >
        <div class="layui-row">
...
<td ><a href="<%=path%>/glyServlet?method=delete&keyid=<%=rs.getString("glyid")%>"  onClick="javascript:if(confirm('是否删除')){return true;} else{return false;}">删除</a>|<a href="<%=path%>/glyServlet?method=modifyindex&keyid=<%=rs.getString("glyid")%>">修改</a>|<a href="<%=path%>/glyServlet?method=detail&keyid=<%=rs.getString("glyid")%>">详细</a></td>
